...
class Exp_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        preds = []
        trues = []
        folder_path = f"./results/{self.args.data_path}/{setting}/"
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if test_data.scale and self.args.inverse:
                    ...
                    if self.args.fanhua:
                        output_scale = test_data.scaler.scale_
                        output_mean = test_data.scaler.mean_
                        batch_y_scale = test_data.scaler.scale_
                        batch_y_mean = test_data.scaler.mean_
                        if outputs.shape[-1] != len(output_scale):
                            raise ValueError(
                                f"输出特征数量 {outputs.shape[-1]} 与 scaler 拟合特征数量 {len(output_scale)} 不一致。"
                            )
