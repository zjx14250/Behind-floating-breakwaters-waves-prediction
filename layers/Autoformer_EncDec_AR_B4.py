...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, additional_attention, self_attention, cross_attention, d_model, c_out=9, d_ff=None,
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None):
        x = x + self.dropout(self.additional_attention(
            x, x, x,
            attn_mask=x_mask
        )[0])
        ...
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))
        y = self.dropout(self.conv2(y).transpose(-1, 1))
        x, trend4 = self.decomp4(x + y)

        residual_trend = trend1 + trend2 + trend3 + trend4
